...
const encodeBase64 = (input: string): string => {
  ...
  while (index < input.length) {
    ...
    const enc3 = Number.isNaN(byte2) ? 64 : (((byte2 as number) & 15) << 2) | ((Number.isNaN(byte3) ? 0 : byte3) >> 6);
...
export const generateAgreementPdfDataUri = (booking: BookingRecord): string => {
  const signatures = booking.agreement?.studentSignatures ?? {};
  const signatureLines = booking.studentNames.map(
    (name) => `${name}: ${signatures[name] ? `Signed at ${signatures[name]}` : 'Pending'}`,
  );
  ...
  const lines = [
    ...
    `Rent: Rs. ${booking.monthlyRent.toLocaleString()} / month`,
